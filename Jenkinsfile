  ...
  stages{
    stage('Provisioning VM on Proxmox with Terraform'){
      steps{
        ...
        //withCredentials([usernamePassword(credentialsId: 'th_digirolamo', passwordVariable: 'xbXif4P82Q88iA==', usernameVariable: 'th_digirolamo')]) {
        withCredentials([usernamePassword(passwordVariable: 'xbXif4P82Q88iA==', usernameVariable: 'th_digirolamo')]) {
